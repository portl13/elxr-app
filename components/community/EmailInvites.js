...
const url = process.env.apiV2 + '/group/email/'

export default function EmailInvites(props) {
  ...
  useEffect(() => {
    if (props?.groupDetails) {
      sentInvitesForm.setFieldValue('group_id', props?.groupDetails?.id)
      sentInvitesForm.setFieldValue('group_name', props?.groupDetails?.name)
      sentInvitesForm.setFieldValue(
        'group_logo',
        props?.groupDetails?.avatar_urls?.thumb
      )
    }
  }, [props])
