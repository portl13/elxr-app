...
function ChannelAudioModalEdit({
  open,
  setOpen,
  id,
  token,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      setIsLoading(true)
      try {
        await genericFetchPost(`${saveAudio}${audio_id}`, token, values)
        await mutateAudiosEdit(values)
        setIsLoading(false)
        setOpen(false)
        alert.success('Podcast Update', TIMEOUT)
      } catch (error) {
  ...
  const { data: audioData } = useSWRImmutable(
    token && audio_id ? [`${saveAudio}${audio_id}`, token] : null,
    getCategories
  )
...
export default ChannelAudioModalEdit
