...
function ChannelVideoModalDelete({ open, setOpen, video , mutateVideos }) {
  ...
  const deleteEvent = async () => {
    ...
    try {
      setLoading(true)
      await genericDelete(`${deleteUrl}/${video.id}/`, token)
      await mutateVideos(video.id)
      setLoading(false)
      setOpen(false)
    } catch (error) {
...
export default ChannelVideoModalDelete
