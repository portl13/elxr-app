...
const deleteUrl = `${process.env.baseUrl}/wp-json/ldlms/v2/sfwd-lessons`

function LessonDeleteModal(props) {
  const alert = useAlert()
  const { user } = useContext(UserContext)
  const token = user?.token
  const { open, setOpen, lesson, mutateLessons } = props
  ...
  const deleteCourse = async () => {
    ...
    try {
      setLoading(true)
      await genericDelete(`${deleteUrl}/${lesson.id}/?force=true`, token)
      await mutateLessons()
      setLoading(false)
      setOpen(false)
    } catch (error) {
  ...
  return (
    <Modal css={style} centered isOpen={open} toggle={toggle}>
      ...
      <ModalBody>
        ...
        <b className="text-primary">{lesson.title.rendered}</b> this action is
        irreversible?
      </ModalBody>
      <ModalFooter>
        <button
