...
function Subscription() {
  ...
  const { data: show_subscription, mutate: show_mutate } = useSWRImmutable(
    token ? [`${productUrl}/show`, token] : null,
    genericFetch
  )
  ...
  useEffect(() => {
    if (show_subscription) {
      formik.setFieldValue('show_subscription', show_subscription.show)
    }
  }, [show_subscription])
