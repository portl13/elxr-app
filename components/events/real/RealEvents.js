...
function RealEvents({ longitude, latitude, eventRealCount, setRealEventCount,
     eventLoader, myEventId, getEventsList,isAllLoading }) {

    const { categoriesRealEvent, startRealEvent, showRealMessage, setShowRealMessage } = useContext(FilterContext)
    ...
    return (
        <div style={{ padding: "0 45px 0 44px" }}>            
            {showRealMessage && <MessajeAlert typeIcon={"warning"} messaje={"No event(s) are available for this location ."} type={"warning"} />}
