...
function HeaderCommunity({ community, isGroup, organizers }) {
    ...
    return (
        <div className="pl-lg-4" css={ProfileCardStyle}>
            <div className="header-cover-image" style={{ backgroundImage: `url(${cover_url})` }}>
                    ...
                    <div className="group-title-wrap connection-group-wrapper">
                        <div className="connection-text">
                            ...
                            <p className="badge-status">
                                <Badge color="info">
                                    <i className={`badge-icon${status !== "public" ? "-danger" : ""}`}>
                                        {status === "public" ? unlock : lock}</i>
                                    {status}
                                </Badge>
                            </p>
                            {types.length !== 0 &&
                        ...
                        <div className="generic-meta generic-group-wrapper generic-org-button">
                            {(!isGroup && is_member && !is_admin) && (
                                <ButtonSmall className="btn">You're {getRoleName(role)}</ButtonSmall>
                            )}
                            {!isGroup && is_admin && (
                                <ButtonSmall
                                    onClick={e => Router.push(`/community/edit/${id}`)}
                                    className="btn">Edit Community</ButtonSmall>
                            )}
                            {isGroup && role &&
                                <ButtonSmallPink className="btn">You're {getRoleName(role)}</ButtonSmallPink>}
                        </div>
                    </div>
                    <div className="group-title-wrap">
                        <p className="w-100 text-center text-lg-left">{raw}</p>
                    </div>

                    {organizers && <div className="generic-group-wrapper">
                        <h4 className="bp-title">Organizer (1)</h4>
                        <div className="user-list-admins">
                            <ul>
                                <li>
                                    <a>
                                        <div className="tooltip-panel">{organizers.name}<em></em></div>
