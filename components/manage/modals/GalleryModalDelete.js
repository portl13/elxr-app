...
function GalleryModalDelete({ open, setOpen, gallery , mutate}) {
    ...
    const deleteEvent = async () => {
        ...
        try {
            setLoading(true)
            await genericDelete(`${deleteUrl}/${gallery.id}/`, token)
            await mutate(gallery.id)
            setLoading(false)
            setOpen(false)
        } catch (error) {
    ...
    return (
        <Modal css={style} centered isOpen={open} toggle={toggle}>
            ...
            <ModalBody>
                are you sure you want to delete the gallery <b className={"text-danger"}>{gallery?.title}</b> this
