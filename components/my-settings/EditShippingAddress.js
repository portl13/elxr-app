...
function EditShippingAddress() {
  ...
  const handlerState = (select) => {
    setStateNew(select);
    formik.setFieldValue("state", select.name);
  };
  ...
  async function getShippingAddressData() {
    const { data } = await getShippingAddress(user);
    ...
    ).filter((e) => e.name === address.state);
    setStateNew({value: stateListNew[0].isoCode, label: stateListNew[0].name})
  }

  useEffect(() => {
    getShippingAddressData().then();
  }, []);

  const updateShippingAddress = (formData) => {
    setShowLoaders(true);
    setShippingAddress(user, formData)
      .then((res) => {
