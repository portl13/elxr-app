...
const deleteUrl = `${process.env.apiV2}/episode`

function EpisodeModalDelete({ open, setOpen, episode , mutate}) {
    ...
    const deleteEvent = async () => {
        ...
        try {
            setLoading(true)
            await genericDelete(`${deleteUrl}/${episode.id}/`, token)
            await mutate(episode.id)
            setLoading(false)
            setOpen(false)
        } catch (error) {
    ...
    return (
        <Modal css={style} centered isOpen={open} toggle={toggle}>
            ...
            <ModalBody>
                are you sure you want to delete the episode <b className={"text-danger"}>{episode?.title}</b> this
...
export default EpisodeModalDelete
