...
export const updateTicket = async (body, user, title = null) => {
  const productData = {
    id: body?.ticket_id,
    name:title ? title : `ticket for ${body?.title}`,
...
export const createdTicket = async (body, user, title = null) => {
  const productData = {
    name: title ? title : `ticket for ${body?.title}`,
    regular_price: `${body?.ticket_price}`,
    description: `${body?.description}`,
...
export const updateKey = async (id, key, value, token) => {
  await axios.post(
    `${baseUrl}/utils/update`,
    {
      id,
