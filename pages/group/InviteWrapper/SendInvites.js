...
import { getmemberDetails, inviteMember, getblockMemberList } from "../../api/member.api";
...
const SendInvites = ({ user, tab, id, getGroupMembers }) => {
    ...
    const loadDetails = (pages, scopes, search, isEmpty = false) => {
        ...
        getmemberDetails(user, data, id).then((res) => {
    ...
    useEffect(() => {
        if (tab === "invites") {
            getblockMemberList(user, { per_page: 100 }).then((res) => {
                ...
                data.push(user.id)
                listAll = [...data, 65222]
                getGroupMemInvite(user, { per_page: 100, group_id: id }).then((resp) => {
                    const respdata = resp.data.map((e) => e.user_id)
                    setBlockedList([...listAll, ...respdata])
                    loadDetails(page, scope, searchText)
                    resp.data.length && setOrgMember(resp.data.map((ele) => ele.user_id))
                }).catch(() => {
                    loadDetails(page, scope, searchText)
                })
            })
    ...
    const sendInvite = () => {
        ...
        inviteMember(user, memberId, formData).then(res => {
    ...
    const handleMsgChange = (e) => {
        ...
        if (text.length <= 500)
            ...
            setInviteMessage(text.substring(0, 500))
    }
                ...
                <div className="invite-section">
                    <div className="inner-section">
                        <div className="panel-tag">
                            ...
                            <div className="custom-checkbox checkbox-panel">
                                ...
                                />
                                <label className="custom-control-label" htmlFor="public">My Connections</label>
                            </div>
                        </div>
                        ...
                        <div className="members-outer-panel">
                            <InfiniteList
                                ...
                                noText={"Members"}
                            >{memberList.length ?
                                memberList.map(members => !checkDisplay(members.id) && <MemberCard
                                    member={members}
                                    memberId={memberId}
                    ...
                    <div className="inner-section">
                        ...
                        <div className="select-invite-container">
                            <div>
                                <div className="select-members-panel">
                                    <div className="info-tag"><span>i</span></div>
                                    <div className="text-tag">Select members to invite
                                        by clicking the + button next to each member.
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="inner-button-panel">
                            <Button onClick={() => sendInvite()}>Send {invideLoad ? <Loader /> : ""}</Button>
                            <Button onClick={() => {
                                setInviteMessage("")
                                setMemberId([])
                                setMemberName([])
