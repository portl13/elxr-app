...
const Photos = ({ groupDetails, status, fetchGroupDetals, id, user }) => {
    ...
    function deleteAvatar() {
        ...
        deleteGroupAvatar(user, id, TYPE).then(() => {
                    ...
                    <section css={DropZoneStyle} className="container">
                        ...
                        {(imageStatus === "upload" && !upload) &&
                            (<div {...getRootProps({ style, className: 'dropzone' })}>
                                ...
                                <input id="browse-button" type="button" value="Select your file" className="btn btn-default"></input>
                            </div>
                            )}
                        {showUpload && <aside style={thumbsContainer}>
                            {thumbs}
                        </aside>}
                                ...
                                <div className="box" style={{ width: "50%", float: "right" }}>
                                    ...
                                    <div className="button-section">
                                        <Button
                                            ...
                                            className="btn btn-primary">Crop Photo
                                            {spiner ? <Loader /> : ""}</Button>
                        ...
                        {imageStatus === "upload" && <p className="message-text">{(status === "photo") ?
                            "If you'd like to delete the existing profile photo but not upload a new one, please use the delete tab."
                            : "For best results, upload an image that is 1950px by 450px or larger."}</p>}
                        {result ?
                            <div className="cover-image-status">
                                <div className="uploader-progress mb-4">
                                    <p id="cover-image-feedback" className="alert alert-success">Your new group {GROUP_NAME} photo was uploaded successfully.</p>
                                </div>
                            </div>
                            : null}
                        ...
                        {delMsg ?
                            <div className="cover-image-status">
                                <div className="uploader-progress mb-4">
                                    <p id="cover-image-feedback" className="alert alert-success">Your group {GROUP_NAME} photo was deleted successfully!</p>
                                </div>
                        ...
                        {imageStatus === "delete" ?
                            <div id="delete-image-container">
                                <p>If you'd like to delete your current {GROUP_NAME} photo, use the delete {GROUP_NAME} Photo button.</p>
                                <button
                                    type="button"
