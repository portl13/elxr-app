...
import { createFriendship, deleteFriendship, followMember, } from "../api/member.api";
import { PENDING, NOT_FRIEND, IS_FRIEND, MEMBER_DEL_ERR, AWAITING, TIMEOUT, ROLES_GROUP_MEM, getProfileRoute } from "../../utils/constant";
import RequestModal from "../../components/requestModal/RequestModal";
import Loader from "../../components/loader";

const MemberListView = ({ id, tab, user, groupDetails, router, setTabCount, tabCount, organizers, setOrganizer }) => {
    ...
    const loadDetails = (pages, search, isEmpty = false) => {
        ...
        getGroupMembers(user, data, id).then((res) => {
            ...
            res.data.forEach((ele) => {
                if (ele.role === "Organizer")
                    memList.Organizer.push(ele)
                else memList.Member.push(ele)
            })
            setMemberList(memList)
            const total = Number(res.headers['x-wp-total']) ? Number(res.headers['x-wp-total']) : 0
            setMemTotal(total)
            const innerNavVal = { ...tabCount }
            innerNavVal.members = total
            if (innerNavtabCountVal.members !== total)
    ...
    const updateState = (member, memberIndex, res) => {
        const status = member.friendship_status === NOT_FRIEND
            ? PENDING : NOT_FRIEND;
        if (memberIndex === "none") {
            const org = { ...organizers }
            org['friendship_status'] = status
            if (res.data.friend_id)
                org['friendship_id'] = res.data.friend_id
            setOrganizer(org)
        } else {
            let list = { ...memberList };
    ...
    const handleReqMember = (data, index) => {
        ...
        if (member.friendship_status === AWAITING) {
            Router.push(getProfileRoute(user.name, user.id, 'connections', 'request'))
            return
        }
        setSpinnerLoad(true)
        const formData = {
        ...
        const getRes = member.friendship_status === PENDING || member.friendship_status === IS_FRIEND ?
            deleteFriendship(user, member.friendship_id) : createFriendship(user, formData);
        getRes.then((res) => updateState(member, memberIndex, res)).catch((err) => {
    ...
    const handleFollowMember = (data, memberIndex) => {
        ...
        setSpinnerLoad(true)
        followMember(user, formData).then((resp) => {
    ...
    const getMemberList = (ele, i) => <MemberList
        data={ele}
        key={ele.id}
    ...
    const getLoadDetails = () => {
        ...
        return <p style={{ textAlign: 'center' }}>{len && !loaderState ? 'No Results.' : ""}</p>
    }
    return (<>
                ...
                <div className="member-container-panel">
                    <div className="member-container-panel">
                        <ActionBar
                            ...
                            isSearch={true}
                            isGroup={true} />
                        <div style={{ textAlign: 'center' }}>{loaderState ? <Loader /> : ""}</div>
                        <div className="d-flex flex-column flex-fill w-100">
                            {ROLES_GROUP_MEM.map((e) => memberList[e].length ?
                                <InfiniteList
                                    ...
                                    <ul className={`members-list ${view === "grid" && "grid"}`}>
                                        <h1 className="members-list-header">{memberList[e].length === 1 ? e : e +"s"}</h1>
        ...
        <RequestModal show={isModalOpen} close={setModalOpen} handleDelete={handleReqMember} showSpinner={spinnerLoad} />
    </>);
};
