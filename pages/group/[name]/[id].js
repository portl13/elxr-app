...
const CommunitiesWrapper = () => {
  ...
  useEffect(() => {
    if (community.id) {
      if (!community.enable_forum)
        setHideDiscussion(true)
      if (community.enable_forum && community.forum)
        setHideDiscussion(false)
    }
  }, [community]);
  ...
  useEffect(() => {
    if (community.id) {
      ...
      if ((!community.is_member) || (
        (community.is_member && !community.is_mod && !community.is_admin && (role === "mods" || role === "admins"))
