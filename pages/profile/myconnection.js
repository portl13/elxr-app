...
export default function MyConnection({ user, parentCallback, setfollowStatus, curntUserId }) {
  ...
  useEffect(() =>
    getConnections()
    , [type, page, curntUserId])
  async function getConnections() {
    await axios(process.env.bossApi + '/members', {
      method: "GET",
      headers: {
  ...
  const handleDelete = (childData) => {
    const id = childData
    axios.delete(process.env.bossApi + '/friends',
      {
        headers: {
          'Authorization': `Bearer ${user?.token}`
        },
  ...
  const followMember = (childData, connectionStatus) => {
    ...
    axios.post(url + `/members/action/${user_id}`,
      {
        "user_id": user_id,
        "action": "follow"
      }
      ...
      .then(res => {
        var index = result.findIndex((item) => item.id == user_id)
  ...
  return (
    <>
      <ActionBar handleActivityChange={handleActivityChange} setView={setView} />
      {loadData === false ? (
        <p css={LoaderContainer}>
          <span><FontAwesomeIcon icon={faClock} /></span>
          Loading your connections. Please wait.</p>
      )
        : null}
      {length == 0 && status && loadData ?
        <p css={LoaderContainer}>
          <span><FontAwesomeIcon icon={faClock} /></span>
          Sorry, there were no connections found.</p>
        : null}
      {loadData === true ?
        <div className="d-flex flex-column flex-fill w-100">
          ...
          >
            <ul className={view === "grid" ? "members-list grid" : "members-list"}>
              {result && result.map(connection => {
                return (
                  <>
                    <MyConnectionCard
      ...
      <div className="pagination">
        <div className="page-count">
          {length == 1 ?
            ...
            :
            length > 1 ?
              <p className="text-right">Viewing 1-{length} of {count} groups</p> : null}
        </div>
      </div>
    </>
  )
