...
function ProfileData({ user, tab, curntUserId, isCurntUser, functionRedirect }) {
  ...
  function getUser() {
    {
      user && Axios.get(profile + curntUserId.id, {
        headers: {
  ...
  useEffect(() => {
    if (tab === "profile") {
      Router.push(functionRedirect(curntUserId.name, curntUserId.id, 'profile'))
      getUser()
    }
  }, [tab, curntUserId])
      ...
      <div className="itemBody">
        <div className="item-body-inner">
          <div className="button-right-container">
            {isCurntUser && <button type="button"
