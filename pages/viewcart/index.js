...
function CartWrapper() {
  ...
  const increaseQuantity=(num, data)=>{
    ...
    setQuantityData(updateCart?{...quantityData,[name]:quantity+1}:{...quantityData})
    ...
    if(index>=0 ){  
      priceData[index].quantity=num===1?1:num-1
          setPriceData([...priceData])      
            ...
            priceData.splice(index,1)       
setPriceData([...priceData,{...data,quantity:num===1?1:num-1}])
                  }
  }
...
useEffect(()=>{
  localStorage.setItem('channelPriceData',JSON.stringify(priceData))}, [priceData])
    ...
    <Layout>
      ...
      <div className="view-cart-container">
        <div className="wcfm-datatable">
          ...
          {priceData&& priceData.map((data,index,arr)=>(  <div className="column-head pink-color">
            
            <div className="view-cart-div-1">
              <span className="cross-icon" onClick={() => {handleDelete(data.id)}}>+</span>
            </div>
            <div className="view-cart-div-2"><img src={data?.images?.[0]?.src} alt="view-cart-images" /></div>
            <div className="view-cart-div-3" onClick={() => alert.success("Coming Soon..", TIMEOUT)}>{data?.name}</div>
            <div className="view-cart-div-4">${data?.price}</div>
            <div className="view-cart-div-5">
              <div className="quantity-tag">
                <span className="left-arrow" onClick={() =>decreaseQuantity(data?.quantity, data)}></span>
                <input type="text" defaultValue={1} value={data?.quantity}  name={data.name} onChange={
                  e=>{
                    const exp = /^[0-9]*$/;
                    if (e.target.value === "" || exp.test(e.target.value)) {
                      setQuantityData({...quantityData,[e.target.name]:Number(e.target.value)})
                      setCartData({...cartData,[e.target.name]:[Number(e.target.value), data.price,data.id, data?.images?.[0]?.src, Number(e.target.value)*data.price] })
                      ...
                      if(index>=0 ){  
                        ...
                        setPriceData([...priceData,{...data,quantity:Number(e.target.value)}])
                      }
                    }else{
                      setQuantityData({...quantityData,[e.target.name]:e.target.value.replace(/\D/g,'')})
                      }
               ...
               />
                <span className="right-arrow" onClick={() => {increaseQuantity(data?.quantity, data)
                }}></span>
              </div>
            </div>
            <div className="view-cart-div-6">${(data?.quantity||0)*data?.price}</div>
          </div>))}
          
          <div className="apply-coupon-container">
            <div className="apply-coupon-tag">
              <input placeholder="Coupon code" type="text" />
              <button className="apply-button" onClick={() => alert.success("Coming Soon..", TIMEOUT)}>Apply button</button>
            </div>
            <button className="update-cart" onClick={()=>setUpdateCart(!updateCart)}>Update cart</button>
          </div>
